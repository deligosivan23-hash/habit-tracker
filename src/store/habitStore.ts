...
interface HabitStore {
  ...
  editLogDuration: (habitId: string, date: string, newDurationSeconds: number) => void;
...
export const useHabitStore = create<HabitStore>()(
  persist(
    (set, get) => ({
            ...
            logs: state.logs.map((l) =>
              l.habitId === habitId && l.date === date
                ? {
                    ...
                    duration: (l.duration ?? 0) + session.duration,
                  }
                : l,
            ),
          }));
      ...
      editLogDuration: (habitId, date, newDurationSeconds) => {
        const clamped = Math.max(0, newDurationSeconds);
        const existing = get().getLog(habitId, date);
        if (existing) {
          set((state) => ({
            logs: state.logs.map((l) =>
              l.habitId === habitId && l.date === date
                ? {
                    ...l,
                    duration: clamped,
                    // Clear sessions when manually overriding — they no longer sum correctly
                    sessions: [],
                  }
                : l,
            ),
